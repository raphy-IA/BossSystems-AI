...
const ServiceDetailPage: React.FC = () => {
  ...
  // We use service.prestations primarily as constants are empty now,
  // we might want to add them to translations later.
  // For now, if empty, we hide the section.
  const featureList = service.prestations;

  return (
    <div className="bg-white">
      <Helmet>
        <title>{t(service.title)} | BOSS SYSTEMS AI</title>
        <meta name="description" content={t(service.description)} />
      </Helmet>
      <div className="bg-[#0A1931] text-white py-16">
        <div className="container mx-auto px-6 text-center">
          <h1 className="text-4xl md:text-5xl font-bold">{t(service.title)}</h1>
          <p className="text-xl text-[#D4AF37] mt-4">{t(service.tagline)}</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2">
            <div className="prose max-w-none text-lg text-gray-700">
              <p className="lead text-xl mb-6">{t(service.description)}</p>
          ...
          <aside className="lg:col-span-1">
            <div className="sticky top-28 bg-gray-50 p-8 rounded-lg shadow-md">
              <h3 className="text-2xl font-bold text-[#0A1931] mb-4">
                {t('services.page.detail.ctaTitle', { service: t(service.title) })}
              </h3>
